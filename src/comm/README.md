...
当前工程内 MCU、按钮通道均设置 `pollIntervalMs = 200`，使用 Tick 模式。FD 模式和 TCP 一样要求调用线程已经有 Looper，但 UART 不创建工作线程，读取和 handler 回调都直接发生在 Looper/Tick 所在线程。
...
```cpp
using McuPool = PacketBufferPoolT<BT_MCU, McuAsk, McuAck>;
```
...
工程当前 MCU、按钮通道传入 `false`，会过滤连续重复包。
...
发送可变载荷协议时，应申请 `BASE_LEN + payloadLen`，由具体 `IAsk` 写包头、命令、长度和载荷并生成校验，最后交给 channel 自动回收。对应 `IAck` 应在长度字段完整后计算总帧长，再由业务 handler 解析命令。
