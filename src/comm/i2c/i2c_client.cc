...
int I2CClient::onI2cData(uint8_t* buf, int len) {
    ...
    for (; offset < len;) {
        ...
        if (mPacketBuff->check(mCurrRecv)) {
            ...
            StringUtils::hexdump("IIC recv data, check fail", mCurrRecv->buf, mCurrRecv->len);
        }
...
#else
    LOG(VERBOSE) << std::to_string(mI2cOpenReq.addr) << " iic send:" << StringUtils::hexStr(ask->buf, ask->len);
    if (mI2cOpenReq.type == I2C_TYPE_SW)
