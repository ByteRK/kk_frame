...
namespace NetworkUtils {
    ...
    struct ResolveResult {
        std::vector<std::string> addresses; // IP 地址
        int errorCode{ 0 };                 // 错误码

        /// @brief 判断解析是否成功且至少获得一个地址
        bool success() const {
            return errorCode == 0 && !addresses.empty();
        }
    };
    ...
    struct InterfaceInfo {
        std::string name;                          // 接口名称(eth0、wlan0...)
        bool isUp{ false };                        // 是否具有 IFF_UP 标志
        bool isRunning{ false };                   // 是否具有 IFF_RUNNING 标志
    ...
    /// @note 阻塞接口：域名解析耗时由系统 DNS 配置决定，不应在 UI 或主循环线程调用
    ResolveResult resolveHost(const std::string& host, AddressFamily family = AddressFamily::Any);
