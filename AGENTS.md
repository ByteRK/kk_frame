...
- [x] Handle UART `POLLERR`, `POLLHUP`, and `POLLNVAL`. UART polling now maps device error events to errno-style diagnostics, closes the fd, releases the device claim, marks the channel disconnected, and emits error/disconnect callbacks. Verified with `./fastCheck.sh`.
- [x] Validate packet-buffer allocation lengths before narrowing them into signed `short` fields. `obtain()` computes with `size_t`, rejects zero or oversized requests, checks allocation failure, and protocol senders and stream decoders handle a null result. Verified with `./fastCheck.sh`.
